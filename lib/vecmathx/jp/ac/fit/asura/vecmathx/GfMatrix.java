 ...
 *
 * @version specification 1.1, implementation $Revision: 1.13 $, $Date:
 *          1999/11/25 10:30:23 $
...
public class GfMatrix implements Serializable {
	...
	public final void mul(GfMatrix m1, GfMatrix m2) {
		...
		if (nRow != m1.nRow)
			throw new ArrayIndexOutOfBoundsException("nRow:" + nRow
					+ " != m1.nRow:" + m1.nRow);
		if (nCol != m2.nCol)
			throw new ArrayIndexOutOfBoundsException("nCol:" + nCol
					+ " != m2.nCol:" + m2.nCol);
		if (m1.nCol != m2.nRow)
	...
	public final void mul(GfVector v1, GfVector v2) {
		...
		if (nCol < v2.getSize())
			throw new IllegalArgumentException("nCol:" + nCol
					+ " < v2.getSize():" + v2.getSize());
	...
	public final void copySubMatrix(int rowSource, int colSource, int numRow,
		...
		if (rowSource < 0 || colSource < 0 || rowDest < 0 || colDest < 0)
			...
			throw new ArrayIndexOutOfBoundsException(
					"Source GMatrix too small.");
		else if (target.nRow < numRow + rowDest
				|| target.nCol < numCol + colDest)
			throw new ArrayIndexOutOfBoundsException(
					"Target GMatrix too small.");

		for (int i = 0; i < numRow; i++)
			for (int j = 0; j < numCol; j++)
				target.elementData[(i + rowDest) * nCol + (j + colDest)] = elementData[(i + rowSource)
						* nCol + (j + colSource)];
	}
	...
	public final void getRow(int row, float array[]) {
		...
		if (array.length < nCol)
			throw new ArrayIndexOutOfBoundsException("array length:"
					+ array.length + " smaller than matrix's nCol:" + nCol);
	...
	public final void getRow(int row, GfVector vector) {
		...
		if (vector.getSize() < nCol)
			throw new ArrayIndexOutOfBoundsException("vector size:"
					+ vector.getSize() + " smaller than matrix's nCol:" + nCol);
	...
	public final void getColumn(int col, float array[]) {
		...
		if (array.length < nRow)
			throw new ArrayIndexOutOfBoundsException("array.length:"
					+ array.length + " < matrix's nRow=" + nRow);
	...
	public final void getColumn(int col, GfVector vector) {
		...
		if (vector.getSize() < nRow)
			throw new ArrayIndexOutOfBoundsException("vector size:"
					+ vector.getSize() + " < matrix's nRow:" + nRow);
		for (int i = 0; i < nRow; i++) {
	...
	public final void get(GfMatrix m1) {
		...
		if (m1.nCol == nCol) {
			...
			for (int i = 0; i < nRow; i++) {
				System.arraycopy(elementData, i * nCol, m1.elementData, i
						* m1.nCol, nCol);
			}
		}
	}
	...
	public final void setRow(int row, float array[]) {
		...
		if (array.length < nCol)
			throw new ArrayIndexOutOfBoundsException("array length:"
					+ array.length + " < matrix's nCol=" + nCol);
	...
	public final int LUD(GfMatrix LU, GfVector permutation) {
		...
		int n = nRow;
		if (n != LU.nRow)
			throw new ArrayIndexOutOfBoundsException("this.nRow:" + n
					+ " != LU.nRow:" + LU.nRow);
		if (n != LU.nCol)
			throw new ArrayIndexOutOfBoundsException("this.nCol:" + n
					+ " != LU.nCol:" + LU.nCol);
		if (permutation.getSize() < n)
			throw new ArrayIndexOutOfBoundsException("permutation.size:"
					+ permutation.getSize() + " < this.nCol:" + n);
